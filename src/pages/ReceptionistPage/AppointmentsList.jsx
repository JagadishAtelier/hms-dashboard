...
import { ChevronLeft, ChevronRight, RefreshCw, Search, Edit2, X, Plus } from "lucide-react";
...
function AppointmentsList() {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentPage, limit, filterStatus, filterVisitType, filterDoctorId, startDate, endDate, sortBy, sortOrder]);
    ...
    try {
      ...
      if (res?.status === "success" && res?.data) {
        ...
        if (payload?.data) {
          ...
          setTotal(payload.total || payload.totalItems || payload.totalRecords || 0);
    ...
    <div className="p-4 sm:p-6 w-full h-full flex flex-col overflow-hidden text-sm">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-xs text-gray-500">Manage appointments — use filters to narrow results</p>
        </div>
            ...
            <div className="min-w-[600px]">
              <table className="w-full table-auto border-collapse">
                ...
                <tbody>
                  {loading ? (
                    <tr>
                      <td colSpan={11} className="py-4 text-center text-gray-500 text-xs">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mt-5 gap-3">
        <p className="text-xs text-gray-500">
          Showing {total === 0 ? 0 : startIndex}-{endIndex} of {total} appointments
        </p>
